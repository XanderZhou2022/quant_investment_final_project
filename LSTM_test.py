...
# one-hot 编码 Regime（假设列名为 'regime'）
if "regime" not in df.columns:
    raise ValueError("数据中缺少 'regime' 列，请确认已经合并 Regime 信息。")

df = pd.get_dummies(df, columns=["regime"])

# 构造输入特征列：因子历史收益 + Regime one-hot
regime_cols = [c for c in df.columns if c.startswith("regime_")]
input_cols = factor_cols + regime_cols

print("输入特征列：", input_cols)
print("Regime 列：", regime_cols)
...
data_input = df[input_cols].values.astype(np.float32)    # 输入特征（因子 + Regime）
...
# 7. 导出预测结果到 CSV
# =========================
output_path = "data/factor_prediction.csv"
