...
\begin{frame}{数据示例：原始因子与指数数据片段}
\small
\begin{itemize}
    \item 选取部分原始数据片段展示本研究所用数据的基本结构：
    \begin{itemize}
        \item 个股与因子层面的原始输入；
        \item 回测过程中使用的关键字段与格式。
    \end{itemize}
\end{itemize}
\vspace{0.5em}
\centering
\includegraphics[width=0.9\textwidth]{figs/原始数据实例.png}
\end{frame}
...
\begin{frame}{市场状态识别所用数据示例}
\small
\begin{itemize}
    \item 本图展示用于 Regime 聚类的上证50指数层面特征数据片段：
    \begin{itemize}
        \item 包括月度收益率、波动率与成交量等列；
        \item 为后续 KMeans 聚类提供输入基础。
    \end{itemize}
\end{itemize}
\vspace{0.5em}
\centering
\includegraphics[width=0.9\textwidth]{figs/市场情况数据.png}
\end{frame}
...
\small
\begin{itemize}
    \item 不同 Regime 下，各风格因子的多空收益在方向和大小上均表现出系统性差异：
    \begin{itemize}
        \item Regime 0：动量类因子整体更强，趋势延续显著；
        \item Regime 1：动量弱化，价值与防御因子占优；
        \item Regime 2：质量因子在震荡环境下更稳健。
    \end{itemize}
\end{itemize}
\centering
\includegraphics[width=0.35\textwidth]{figs/regime.png}
\end{frame}
...
\begin{frame}{特征标准化与归一化示例}
\small
\begin{itemize}
    \item 在进入回测前，对多维特征进行标准化/归一化处理：。
\end{itemize}
\vspace{0.5em}
\centering
\includegraphics[width=0.9\textwidth]{figs/归一化实例.png}
\end{frame}


\begin{frame}{回测示意：策略净值与关键字段}
\small
\begin{itemize}
    \item 展示回测结果中的典型记录格式与净值/收益率曲线示意：
\end{itemize}
\vspace{0.5em}
\centering
\includegraphics[width=0.9\textwidth]{figs/回测实例.png}
\end{frame}
...
\hline
策略 & 年化超额收益 & Tracking Error & IR & CAPM Alpha(年化) \\
\hline
等权配置 & 0.0909 & 0.0733 & 1.0552 & 0.0910 \\
动态权重(有状态) & 0.2455 & 0.1743 & 1.2547 & 0.2014 &  \\
动态权重(无状态) & \textbf{0.2794} & 0.1600 & \textbf{1.4792} & \textbf{0.2363}  \\
...
\small
\begin{itemize}
    \item 四策略 Information Ratio 柱状图。
\end{itemize}
\vspace{0.6em}
\centering
% TODO: Replace with your exported figure path
...
\small
\begin{itemize}
    \item 四策略（第5组）回撤曲线图，用于展示尾部风险差异与修复速度。
\end{itemize}
\vspace{0.6em}
\centering
% TODO: Replace with your exported figure path
...
\begin{frame}{Sharpe 对比}
\small
\begin{itemize}
    \item 四策略 Sharpe Ratio 柱状图,与上页 IR 柱状图构成“风险调整后收益 + 相对基准”双证据）。
\end{itemize}
\vspace{0.6em}
\centering
% TODO: Replace with your exported figure path
